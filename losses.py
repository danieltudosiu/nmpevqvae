...
class AdaptiveLoss(object):
    ...
    def __call__(self, network_outputs):
        ...
        summaries.append({"scalar_value": adaptive_loss, "tag": "Adaptive-Loss"})

        summaries.append(
            {"scalar_value": self.lambda_reconstruction, "tag": "Lambda-Reconstruction"}
        )

        for key in network_outputs:
            if "q" in key:
                if loss_total is None:
                    loss_total = network_outputs[key]["ql"]
                else:
                    loss_total += network_outputs[key]["ql"]

                summaries.append(
                    {
                        "scalar_value": network_outputs[key]["ql"],
                        "tag": "L2-VQ_" + str.upper(str(key)) + "-Loss",
                    }
                )

        loss_total += adaptive_loss.mean()

        summaries.append({"scalar_value": loss_total, "tag": "Total_Loss"})
...
class BaurLoss(object):
    def __init__(self, lambda_reconstruction=1, lambda_gdl=0):
    ...
    def __call__(self, network_outputs):
        ...
        summaries["scalar"]["L1-Reconstruction-Loss"] = l1_reconstruction
        summaries["scalar"]["L2-Reconstruction-Loss"] = l2_reconstruction
        summaries["scalar"]["Lambda-Reconstruction"] = self.lambda_reconstruction
        ...
        summaries["scalar"]["L1-Image_Gradient-Loss"] = l1_gdl
        summaries["scalar"]["L2-Image_Gradient-Loss"] = l2_gdl
        summaries["scalar"]["Lambda-Image_Gradient"] = self.lambda_gdl
        ...
        for key in network_outputs:
            if "q" in key:
                ...
                summaries["scalar"][
                    "L2-VQ_" + str.upper(str(key)) + "-Loss"
                ] = network_outputs[key]["ql"]

        loss_total += l1_reconstruction + l2_reconstruction + l1_gdl + l2_gdl

        summaries["scalar"]["Total_Loss"] = loss_total

        return loss_total, {"summaries": summaries}
    ...
    def __image_gradients(image):
        """
            Ported from Tensorflow https://github.com/tensorflow/tensorflow/blob/v2.1.0/tensorflow/python/ops/image_ops_impl.py#L3442
        """
        ...
        dzz = tensor(()).new_zeros(
            (batch_size, features, 1, height, width), device=image.device
        )
        dz = cat([dz, dzz], 2)
        dz = reshape(dz, input_shape)

        dyz = tensor(()).new_zeros(
            (batch_size, features, depth, 1, width), device=image.device
        )
        dy = cat([dy, dyz], 3)
        dy = reshape(dy, input_shape)

        dxz = tensor(()).new_zeros(
            (batch_size, features, depth, height, 1), device=image.device
        )
        dx = cat([dx, dxz], 4)
