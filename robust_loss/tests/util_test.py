...
class UtilTest(tf.test.TestCase):
    ...
    def testInvSoftplusIsCorrect(self):
        ...
        x = np.float32(np.exp(np.linspace(-10.0, 10.0, 1000)))
    ...
    def testLogitIsCorrect(self):
        ...
        x = np.float32(np.linspace(1e-5, 1.0 - 1e-5, 1000))
    ...
    def testAffineSigmoidSpansRange(self):
        ...
        for _ in range(10):
            lo = np.random.uniform(0.0, 0.3)
            hi = np.random.uniform(0.5, 4.0)
    ...
    def testAffineSigmoidIsCentered(self):
        ...
        for _ in range(10):
            lo = np.random.uniform(0.0, 0.3)
            hi = np.random.uniform(0.5, 4.0)
            y = util.affine_sigmoid(np.array(0.0), lo=lo, hi=hi)
    ...
    def testAffineSoftplusSpansRange(self):
        ...
        x = np.finfo(np.float32).max * np.array([-1, 1], dtype=np.float32)
        for _ in range(10):
            lo = np.random.uniform(0.0, 0.1)
            ref = np.random.uniform(0.2, 10.0)
    ...
    def testAffineSoftplusIsCentered(self):
        ...
        for _ in range(10):
            lo = np.random.uniform(0.0, 0.1)
            ref = np.random.uniform(0.2, 10.0)
            y = util.affine_softplus(np.array(0.0), lo=lo, ref=ref)
    ...
    def testDefaultAffineSigmoidMatchesSigmoid(self):
        ...
        x = np.float32(np.linspace(-10.0, 10.0, 1000))
    ...
    def testStudentsTNllAgainstTfp(self):
        ...
        for _ in range(10):
            x = np.random.normal()
            df = np.exp(4.0 * np.random.normal())
            scale = np.exp(4.0 * np.random.normal())
            nll = util.students_t_nll(x, df, scale)
            nll_true = -tfp.distributions.StudentT(
                df=df, loc=tf.zeros_like(scale), scale=scale
            ).log_prob(x)
    ...
    def testRgbToSyuvPreservesVolume(self):
        ...
        for _ in range(4):
            ...
            self.assertAllClose(det, 1.0, atol=1e-5, rtol=1e-5)
