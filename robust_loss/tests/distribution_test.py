...
class TestDistribution(parameterized.TestCase):
    ...
    def testSplineCurveIsC1Smooth(self):
        ...
        x1 = np.linspace(0.0, 8.0, 10000, dtype=np.float64)
        x2 = x1 + 1e-7
    ...
    @parameterized.named_parameters(("Single", np.float32), ("Double", np.float64))
    ...
    def testAlphaZeroSamplesMatchACauchyDistribution(self, float_dtype):
        ...
        ks_statistic = scipy.stats.kstest(
            samples, "cauchy", (0.0, scale * np.sqrt(2.0))
        ).statistic
    ...
    def testAlphaTwoSamplesMatchANormalDistribution(self, float_dtype):
        ...
        scale = float_dtype(1.7)
        samples = self._distribution.draw_samples(
            2.0 * np.ones(num_samples, dtype=float_dtype),
            scale * np.ones(num_samples, dtype=float_dtype),
        )
        # Perform the Kolmogorov-Smirnov test against a normal distribution.
        ks_statistic = scipy.stats.kstest(samples, "norm", (0.0, scale)).statistic
        np.testing.assert_(ks_statistic < 0.01)
    ...
    def testAlphaZeroNllsMatchACauchyDistribution(self, float_dtype, device):
        ...
        x = np.linspace(-10.0, 10, 1000, dtype=float_dtype)
        scale = np.array(1.7, float_dtype)
        alpha = np.array(0.0, float_dtype)
        nll = (
            self._distribution.nllfun(
        ...
        nll_true = -scipy.stats.cauchy(0.0, scale * np.sqrt(2.0)).logpdf(x)
    ...
    def testAlphaTwoNllsMatchANormalDistribution(self, float_dtype, device):
        ...
        x = np.linspace(-10.0, 10, 1000, dtype=float_dtype)
        scale = np.array(1.7, float_dtype)
        alpha = np.array(2.0, float_dtype)
        nll = (
            self._distribution.nllfun(
                torch.tensor(x, device=device),
                torch.tensor(alpha, device=device),
        ...
        nll_true = -scipy.stats.norm(0.0, scale).logpdf(x)
        np.testing.assert_allclose(nll, nll_true, atol=1e-6, rtol=1e-6)

    @parameterized.named_parameters(("Single", np.float32), ("Double", np.float64))
    def testPdfIntegratesToOne(self, float_dtype):
        """Tests that the PDF integrates to 1 for different alphas."""
        alphas = np.exp(np.linspace(-4.0, 8.0, 8, dtype=float_dtype))
        scale = float_dtype(1.7)
        x = np.arange(-128.0, 128.0, 1 / 256.0, dtype=float_dtype) * scale
        for alpha in alphas:
            nll = (
                self._distribution.nllfun(x, np.array(alpha), np.array(scale))
                .detach()
                .numpy()
            )
            pdf_sum = np.sum(np.exp(-nll)) * (x[1] - x[0])
            np.testing.assert_allclose(pdf_sum, 1.0, atol=0.005, rtol=0.005)
